//movements, walls, bullets, dash, audio, targets, score, map
use bevy::prelude::*;
...
const MIN_SPAWN_DISTANCE: f32 = 150.0;
...
const TARGET_COLOR: Color = Color::srgb(0.2, 0.8, 0.3);
...
fn main() {
    App::new()
        ...
        .add_systems(Startup, (setup_camera, load_assets, spawn_walls, spawn_ui))
        .add_systems(Update, (
            menu_system,
            game_input,
            player_movement, 
...
fn spawn_target(mut commands: Commands, player_query: Query<(Entity, &Transform), With<Player>>) {
    use rand::Rng;
    let mut rng = rand::rng();
    
    // Get player position if available, otherwise use origin
    let player_pos = player_query.single().map(|(_, t)| t.translation).unwrap_or(Vec3::ZERO);
    
    // Parse map to find valid spawn positions (non-wall tiles)
    let lines: Vec<&str> = LEVEL_MAP.lines().collect();
    let height = lines.len() as f32;
    let width = if let Some(line) = lines.first() {
        line.len() as f32
    ...
    for (row, line) in lines.iter().enumerate() {
        for (col, ch) in line.chars().enumerate() {
            if ch == '.' {
                let x = offset_x + (col as f32 * TILE_SIZE);
                let y = offset_y - (row as f32 * TILE_SIZE);
                valid_positions.push(Vec3::new(x, y, 0.0));
            }
        }
    }
    
    // Filter positions that are far enough from player
    let far_positions: Vec<Vec3> = valid_positions
        .into_iter()
        .filter(|pos| player_pos.distance(*pos) >= MIN_SPAWN_DISTANCE)
        .collect();
    
    // Pick random position from valid ones
    let spawn_pos = if !far_positions.is_empty() {
        far_positions[rng.random_range(0..far_positions.len())]
    } else {
        Vec3::new(100.0, 100.0, 0.0) // Fallback
    };
    ...
    if input.just_pressed(KeyCode::Space) {
        match game.state {
            GameState::Menu => {
                ...
                spawn_target(commands.reborrow(), player_query);
            }
            GameState::GameOver => {
                // Restart game - clean up first
    ...
    for (p_e, p_t) in proj_q.iter() {
        ...
        for (t_e, t_t) in target_q.iter() {
            ...
            if dx < (p_rad + t_half) && dy < (p_rad + t_half) {
                ...
                // Spawn new target using the grid system
                use rand::Rng;
                let mut rng = rand::rng();
                
                let player_pos = player_query.single().map(|t| t.translation).unwrap_or(Vec3::ZERO);
                
                // Parse map to find valid spawn positions
                let lines: Vec<&str> = LEVEL_MAP.lines().collect();
                let height = lines.len() as f32;
                let width = if let Some(line) = lines.first() {
                    line.len() as f32
