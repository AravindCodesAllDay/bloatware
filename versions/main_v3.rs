//movements, walls, bullets, dash, audio, targets, score
use bevy::prelude::*;
...
fn spawn_target(mut commands: Commands, player_query: Query<(Entity, &Transform), With<Player>>) {
    ...
    // Define safe spawn area (inside walls with padding)
    let min_x = -900.0 / 2.0 + WALL_THICKNESS + TARGET_SIZE;
    let max_x = 900.0 / 2.0 - WALL_THICKNESS - TARGET_SIZE;
    let min_y = -600.0 / 2.0 + WALL_THICKNESS + TARGET_SIZE;
    let max_y = 600.0 / 2.0 - WALL_THICKNESS - TARGET_SIZE;
    
    // Keep trying random positions until we find one far enough from player
    ...
    for _ in 0..100 {
        ...
        if player_pos.distance(spawn_pos) >= MIN_SPAWN_DISTANCE {
            break;
        }
    }
